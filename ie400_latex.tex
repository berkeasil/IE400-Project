...
\medskip
\textbf{Extended Result (Teleport Allowed).}
If the movement constraint (C3) is relaxed for the transition $t=2\to t=3$ only ---
i.e.\ Lara is permitted a ``ghost teleport'' at that single step --- the checkpoint
constraint $x_{3,4,4}=1$ becomes feasible. Formally, constraint (C3) is enforced for all
$t \neq 2$; the adjacency requirement between $t=2$ and $t=3$ is dropped. The model is
re-solved under this relaxation with the same objective and all remaining constraints
intact.

\textbf{Result: } $T^*=14$ steps. Lara waits at $(1,1)$ for two time steps, teleports to
$(4,4)$ at $t=3$, then proceeds to the exit along a camera-free path.
